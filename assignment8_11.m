...
Ipts2 = [Ipts2X';Ipts2Y'];
    % RANSAC
    [H, inliers] = ransacfithomography(Ipts1, Ipts2, 0.001);
    ...
    for n = inliers
        line([Ipts1(2,n) Ipts2(2,n)], [Ipts1(1,n)+shiftY Ipts2(1,n)+shiftY],'color',[1 1 0])
    end
    ...
    inliersText = sprintf('Inliers: %d (%d%%) \n',length(inliers),round(100*length(inliers)/length(Ipts1)));
    ...
    text(25,shiftY-25,'Andrew Petersen','fontsize',16,'color',white,'fontweight','bold');
